/**
 * Configuration management for the Mikey extension
 * 
 * This module provides a centralized way to access and manage VS Code workspace
 * and user configuration settings for the Mikey extension.
 * 
 * @remarks
...
/**
 * Configuration interface for Mikey extension
 */
export interface MikeyConfiguration {
 ...
 */
const DEFAULT_CONFIG: MikeyConfiguration = {
 ...
 * 
 * This singleton class provides type-safe access to all Mikey extension configuration.
...
export class ConfigManager {
  ...
  private config: MikeyConfiguration;
  ...
  private constructor() {
    ...
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("mikey")) {
          this.config = this.loadConfiguration();
        }
      }),
    );
   ...
   */
  private loadConfiguration(): MikeyConfiguration {
    const vsConfig = vscode.workspace.getConfiguration("mikey");

    return {
      prdFilePath: vsConfig.get<string>(
        "prdFilePath",
        DEFAULT_CONFIG.prdFilePath,
      ),
      skillsDirectory: vsConfig.get<string>(
        "skillsDirectory",
        DEFAULT_CONFIG.skillsDirectory,
      ),
      testSkillsDirectory: vsConfig.get<string>(
        "testSkillsDirectory",
        DEFAULT_CONFIG.testSkillsDirectory,
      ),
      promptsDirectory: vsConfig.get<string>(
        "promptsDirectory",
        DEFAULT_CONFIG.promptsDirectory,
      ),
      promptTemplate: vsConfig.get<string>(
        "promptTemplate",
        DEFAULT_CONFIG.promptTemplate,
      ),
      featureBranchPrefix: vsConfig.get<string>(
        "featureBranchPrefix",
        DEFAULT_CONFIG.featureBranchPrefix,
      ),
      mainBranch: vsConfig.get<string>("mainBranch", DEFAULT_CONFIG.mainBranch),
      categories: vsConfig.get<string[]>(
        "categories",
   ...
   */
  public getConfig(): Readonly<MikeyConfiguration> {
    return { ...this.config };
  }
